...
namespace Application.Test
{
    class CrearCreditoServiceTest
    {
        ...
        CrearCreditoService crearCreditoService;
        [SetUp]
        public void SetUp()
        {
            ...
            crearEmpleadoService.Ejecutar(request);
                                        ...
                                        int plazo, double tasaDeInteres,string esperado)
        {
            ...
            var response = crearCreditoService.Ejecutar(request);
        ...
        [TestCaseSource("TestDataIncorrecto")]
        public void CrearCreditoIncorrecto(string cedulaEmpleado, string numero, double valor,
                                        int plazo, double tasaDeInteres, string esperado)
        {
            var request = new CrearCreditoRequest { CedulaEmpleado = cedulaEmpleado, Numero = numero, Plazo = plazo, TasaDeInteres = tasaDeInteres, Valor = valor };
            var response = crearCreditoService.Ejecutar(request);
        ...
        private static IEnumerable TestDataIncorrecto()
        {
        ...
        {
            var request = new CrearCreditoRequest { CedulaEmpleado = cedulaEmpleado, Numero = numero, Plazo = plazo, TasaDeInteres = tasaDeInteres, Valor = valor };
            crearCreditoService.Ejecutar(request);
            var response = crearCreditoService.Ejecutar(request);
            string obtenido = response.Mensaje.Contains(esperado) ? esperado : String.Join(',', response.Mensaje);
            Assert.AreEqual(esperado, obtenido);
        }
